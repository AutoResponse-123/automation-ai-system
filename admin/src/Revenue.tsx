...
import { PLAN_PRICE, PLAN_COLORS_HEX as PLAN_COLORS, DEFAULT_PLAN } from './plans'
  ...
  async function load() {
    ...
    if (businesses) {
      const cs = await Promise.all(businesses.map(async b => {
        ...
        return { id: b.id, name: b.name, tokens, cost: tokens * 0.000003, msgs: msgCount, plan: b.plan || DEFAULT_PLAN }
      }))
