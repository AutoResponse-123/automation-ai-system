...
const CAT_COLORS = ['#1585c7','#22a7f0','#38bdf8','#f59e0b','#f87171','#e879f9','#fb923c','#4fc3f7']
  ...
  return (
    <div style={s.chartCard}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
        <div style={s.legendItem}><div style={{ ...s.legendDot, background: '#1585c7' }} />Usuario</div>
        <div style={s.legendItem}><div style={{ ...s.legendDot, background: '#22a7f0' }} />IA</div>
      </div>
      <div style={s.barsArea}>
        {stats.map((d, i) => {
            ...
            <div key={d.date} style={s.barCol}
              ...
              <div style={s.barTrack}>
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, display: 'flex', flexDirection: 'column', height: `${userPct + botPct}%`, transition: 'height 0.3s' }}>
                  <div style={{ flex: botPct, background: isHovered ? '#22a7f0' : '#22a7f099', borderRadius: '3px 3px 0 0', transition: 'background 0.15s' }} />
                  <div style={{ flex: userPct, background: isHovered ? '#1585c7' : '#1585c799', transition: 'background 0.15s' }} />
                </div>
              </div>
              <div style={{ ...s.barLabel, color: isHovered ? 'var(--text-2)' : 'var(--text-faint)', transition: 'color 0.15s' }}>
                {formatDate(d.date)}
  ...
  return (
    <div style={s.chartCard}>
            ...
            <div key={h.hour} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, height: '100%', cursor: 'default' }}
              ...
              <div style={{ flex: 1, width: '100%', position: 'relative', background: 'var(--bg-input)', borderRadius: 2 }}>
                <div style={{
                  ...
                  height: `${pct}%`,
                  background: isHot ? '#f59e0b' : '#1585c7',
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
        ...
        <div style={s.legendItem}><div style={{ ...s.legendDot, background: '#1585c7' }} />Normal</div>
      </div>
    </div>
  )
}
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={s.summaryGrid}>
            ...
            <SummaryCard label={t('analytics_avg_automation')} value={`${avgAutomation}%`} color="#22a7f0" />
          ...
          <div style={s.chartsGrid}>
            ...
            <BarChart stats={stats} valueKey="automationRate" color="#22a7f0" label={t('analytics_chart_automation')} format={v => `${v}%`} />
          </div>
          ...
          <div style={{ ...s.chartsGrid, marginTop: 10 }}>
            <BarChart stats={stats} valueKey="tokens" color="#1585c7" label={t('analytics_chart_tokens')} format={v => v >= 1000 ? `${(v/1000).toFixed(1)}k` : String(v)} />
                  ...
                  <tbody>
                    {contacts.map((c, i) => (
                      <tr key={c.phone} style={{ borderBottom: '0.5px solid #0d0d20' }}>
                        ...
                        <td style={{ padding: '5px 0 5px 8px', color: '#1585c7', fontWeight: 500, textAlign: 'right' as const }}>{c.interaction_count}</td>
...
const s: Record<string, React.CSSProperties> = {
  ...
  rangeBtnActive: { background: 'var(--bg-card)', borderColor: 'var(--border-mid)', color: '#1585c7' },
